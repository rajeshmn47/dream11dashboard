...
export function sameDayorNot(a, b) {
  ...
  if (first.getDate() == second.getDate() && first.getDay() == second.getDay()) {
    return true;
  }
}
...
export function hoursRemaining(date, i, d) {
  ...
  const c = Math.floor(b.getTime() / (60 * 60 * 1000) - a.getTime() / (60 * 60 * 1000));
  const z = b.getTime() / (60 * 60 * 1000) - a.getTime() / (60 * 60 * 1000);
  if (z > 1) {
    const e = getMinutes(z);
    const s = sameDayorNot(new Date(), new Date(date)) || isTommorrow(new Date(), new Date(date));
